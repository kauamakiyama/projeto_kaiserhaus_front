...
const Sobremesas: React.FC = () => {
  ...
  const carregar = async () => {
    ...
    try {
      ...
      const ehSobremesaProduto = (p: any) => {
        ...
        return (catIdStr && sobremesaIds.has(catIdStr)) || nomeEmb === "sobremesas";
      };

      const somenteSobremesas = produtosApi.filter(ehSobremesaProduto);

      const produtosFormatados: Produto[] = somenteSobremesas
        .map((prod: any) => {
          const catIdStr =
  ...
  const handleSalvarNovoProduto = async () => {
    try {
      ...
      const catSobremesas = categoriasApi.find((cat: any) => {
      ...
      if (!catSobremesas) {
        alert("Categoria 'Sobremesas' não encontrada.");
        return;
      }

      const categoria_id =
        toIdStr(catSobremesas._id) ??
        toIdStr(catSobremesas.id) ??
        catSobremesas._id ??
        catSobremesas.id;
      ...
      alert("Sobremesa adicionada com sucesso!");
  ...
  const handleExcluirProduto = async (produto: Produto) => {
    ...
    try {
      ...
      alert("Sobremesa excluída com sucesso!");
    } catch (error) {
    ...
    <>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <textarea
                  ...
                  placeholder="Descrição da sobremesa"
                  rows={4}
                  required
                />
              </div>
      ...
      {modalAdicionarAberto && (
        <div className="modal-overlay" onClick={handleFecharModalAdicionar}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Adicionar Nova Sobremesa</h2>
